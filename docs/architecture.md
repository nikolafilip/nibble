...
| Instruction register, operand register, control | Holds the current instruction and its 8-bit operand; a five-state step counter, two decoders (opcode, and the operand nibble for the 0000 family) and a diode matrix (45 rows: every instruction's first execute step, the second step of the four memory accesses and of the six free opcodes, the four conditional jumps) turn them into control-line pulses. New instructions are diodes. | `03-sequencer` |
