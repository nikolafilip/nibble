...
def main():
    ...
    x,y=16*G,30*G
    w.symbol("Connector","USB_B",w.ref('J'),"USB_B",x,y,0,('1','2','3','4','SH'),"Connector_USB:USB_B_OST_USB-B1HSxx_Horizontal",[("Description","USB-B receptacle, power only",True)],sim=False)
    ...
    w.symbol("Device","Polyfuse",w.ref('F'),"750mA",fx,y-1.5*G,0,('1','2'),"Fuse:Fuse_BelFuse_0ZRE0075FF_L11.5mm_W4.8mm",[("Description","resettable fuse",True)],sim=False)
    ...
    # power LED
    lx=fx+22*G
    w.PW('+5V',lx,y-4*G); w.W(lx,y-4*G,lx,y-3*G); w.R('1k',lx,y-1.5*G); w.W(lx,y,lx,y+0.5*G); w.LED(lx,y+2*G); w.W(lx,y+3.5*G,lx,y+5*G); w.PW('GND',lx,y+5*G); w.T("power",lx+1.5*G,y+2*G,1.0)
    # bulk + decoupling
    frame.decoupling(w,lx+8*G,y,values=(('100n',False),('100u',True)))
    # bus pull-ups
    px=lx+24*G
    w.T("Bus pull-ups (the only ones in the machine)",px-2*G,y-6*G,1.27)
    for i in range(4):
        xx=px+i*5*G; w.PW('+5V',xx,y-4*G); w.W(xx,y-4*G,xx,y-3*G); w.R('10k',xx,y-1.5*G); w.W(xx,y,xx,y+2*G); w.L(f'BUS{i}#',xx,y+2*G,270,'bidirectional')
    # test loops on bus lines + CLK
    tx=px+24*G
    w.T("Test loops",tx-2*G,y-6*G,1.27)
    for k,n in enumerate(['BUS0#','BUS1#','BUS2#','BUS3#','CLK','RST']):
        xx=tx+k*5*G; w.testpoint(n,xx,y); w.W(xx,y,xx,y+2*G); w.L(n,xx,y+2*G,270,'input')
