...
def main():
    ...
    w.T(f"{d.ntransistors()} transistors, {d.nresistors()} resistors, {d.nleds()} LEDs, 16 DIP-8 switches, 128 diodes.\n"
        "From the bus header: PC7..PC0.   To the bus header: M7..M0 through diodes (the hub holds M low; several pages OR together).\n"
    ...
    for i in range(4):
        ...
        j=w.pinheader(3,x,y,f"PAGE{i}"); w.at(j,140+i*8,10,0)
    ...
    w.label("PAGE jumpers 3 2 1 0",138,5,1.0)
    ...
    BW=ax0+2*40+4; BH=max(w.pcb_extent[1],24+8*27)+4
    ksch.write_plan(w,os.path.join(OUT,f'{PROJECT}.plan.json'),(BW,BH),extra=dict(silk_big=[("NIBBLE PROGRAM MEMORY",BW-52,3,1.8)],hide_refs=['Q','R','D']))
