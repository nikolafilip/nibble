"""Parse a kicad-cli kicadsexpr netlist into {net: [(ref,pin),...]} and {ref: (value, footprint, dnp)}."""
import re
def parse(path):
    s=open(path).read()
    nets={}
    ...
    for m in re.finditer(r'\(comp\s*\(ref "([^"]+)"\)(.*?)\n\t\t\)',s,re.S):
        ...
        comps[m.group(1)]=(v.group(1) if v else '', f.group(1) if f else '', '(name "dnp")' in b)
    return nets,comps
