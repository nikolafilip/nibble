...
memory instruction). A 4-to-16 decoder selects the slot; per slot a write line WR = slot AND MI and a read line
RD = slot AND MO. A cell is a pair of cross-coupled inverters (the latch) plus two write pull-downs that force it
...
GROUP_TITLES={
 'clock':'TWO-PHASE CLOCK: only PH1 is used here, to close the address latches before the rising edge',
 ...
 'wl':'WORD LINES: WRn = slot n AND MI (write), RDn = slot n AND MO (read)',
...
def build():
    ...
    d.group='wl'
    d.inv('MIN','MI',pu='22k'); d.inv('MON','MO',pu='22k')
