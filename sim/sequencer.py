...
OPCODE={1:'LDI',2:'JMP',3:'JC',4:'JZ',5:'LOAD',6:'STORE',7:'CALL',8:'JNZ',9:'JNC'}
FAMILY={0:'NOP',1:'MOV',2:'XCH',3:'ADD',4:'SUB',5:'OUT',6:'HLT',7:'DEC',8:'INC',9:'LOADB',10:'STOREB',11:'IN',12:'RET',13:'AND',14:'OR',15:'XOR'}
TWO_WORD=(2,3,4,7,8,9)
# the matrix, exactly as docs/isa.md: instruction -> (S3 lines, S4 lines). The conditional jumps put PCL on their flag-gated rows.
RECIPE={'NOP':({'DONE'},set()),'MOV':({'BA','DONE'},set()),'XCH':({'AB','BA','DONE'},set()),
        ...
        'CALL':({'RAI','PCL','DONE'},set()),'JNZ':({'DONE'},set()),'JNC':({'DONE'},set())}
...
GROUP_TITLES={
 ...
 'opdec':'OPCODE DECODER: IR3..0 -> OP0#..OP15# (active low), through a pair of 2-to-4 predecoders so no NAND has more than two transistors in series.  TWO = JMP or JC or JZ or CALL or JNZ or JNC.',
...
def build():
    ...
    d.nand('TWO_A','OP2#','OP3#','OP4#'); d.inv('TWO_AN','TWO_A'); d.nand('TWO_B','OP7#','OP8#','OP9#'); d.inv('TWO_BN','TWO_B'); d.nand('TWO','TWO_AN','TWO_BN'); d.inv('TWON','TWO')
    d.group='famdec'
    ...
    d.nor('R_JCC','OP3#','T3N','CFQ_qn',pu='10k'); d.nor('R_JZZ','OP4#','T3N','ZFQ_qn',pu='10k')     # jump if the flag is set: the row needs the flag's complement low
    d.nor('R_JNZZ','OP8#','T3N','ZFQ',pu='10k'); d.nor('R_JNCC','OP9#','T3N','CFQ',pu='10k')          # jump if clear (D044): the flag itself
    # ---- the matrix ----
    d.group='matrix'
    for row,name,step in rows:
        if name is None: continue
        for col in sorted(RECIPE[name][step-3],key=COLS.index): d.diode(row,f'{col}_m')
    for r in ('R_JCC','R_JZZ','R_JNZZ','R_JNCC'): d.diode(r,'PCL_m')
    d.group='cols'
...
if __name__=='__main__':
    d=build(); print(d.check()); print('transistors',d.ntransistors(),'resistors',d.nresistors(),'diodes',d.ndiodes(),'leds',d.nleds(),'rows',len(d.rows)+4)
